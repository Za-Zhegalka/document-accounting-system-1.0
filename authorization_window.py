import sqlite3
from PyQt6.QtWidgets import QApplication, QMainWindow, QLineEdit, QPushButton, QVBoxLayout, QWidget, QComboBox, \
    QMessageBox, QInputDialog
from admin_window import AdminWindow
from operator_window import OperatorWindow
from user_window import UserWindow
from external_user_window import ExternalUserWindow
...
class AuthorizationWindow(QMainWindow):
    ...
    def login(self):
        ...
        if count == 1:
            ...
            if role == "Администратор":
                QMessageBox.information(self, "Успешная авторизация",
                                        f"Вы успешно авторизовались под \"{username}\" с правом доступа \"{role}\"")
                self.close()
                ...
                QMessageBox.information(self, "Успешная авторизация",
                                        f"Вы успешно авторизовались под \"{username}\" с правом доступа \"{role}\"")
                self.close()
                external_actor_window = ExternalUserWindow(username, role, self)
    ...
    def create_user(self):
        ...
        if ok:
            ...
            if ok:
                role, ok = QInputDialog.getText(self, "Создание пользователя", "Введите роль:")
                if ok:
                    create_user_query = """
                            INSERT INTO users (username, password, role) VALUES (?, ?, ?)
                        """
                    self.cursor.execute(create_user_query, (username, password, role))
                    self.connection.commit()
                    self.refresh_user_combo()